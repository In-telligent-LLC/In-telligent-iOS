...
class NotificationFilterView: UIView {
    ...
    var filter: NotificationFilter = .none {
        didSet {
            setupFilterButton()
    ...
    @objc func changeFilterTapped() {
        let vc = SelectNotificationFilterViewController.vc(filter: filter)
        vc.delegate = self
...
extension NotificationFilterView: SelectNotificationFilterViewControllerDelegate {
